...
public class BackendApplication {
	...
	ApplicationRunner init(RightsTypeRepository rightsTypeRepository, RightRegistrationRepository rightRegistrationRepository,
						   ProvinceRepository provinceRepository,HostpitalRepository hostpitalRepository) throws Exception {
		...
		return args -> {
			...
			Stream.of("รพ.นครราชสีมา","รพ.กรุงเทพ","testhostpital").forEach(hostpitalName -> {
				...
				hostpital.setHostpitalName(hostpitalName);
				hostpitalRepository.save(hostpital);
			});
			...
			Stream.of("Graph", "Sun").forEach(userName -> {
				...
				if (userName == "Graph") {
					...
					hostpitalid = hostpitalRepository.findByHostpitalName("รพ.กรุงเทพ");
					rightRegistration.setHostpital(hostpitalid);
					...
					hostpitalid = hostpitalRepository.findByHostpitalName("รพ.นครราชสีมา");
					rightRegistration.setHostpital(hostpitalid);
			...
			hostpitalRepository.findAll().forEach(System.out::println);
			};
